...
bool ConfigurationReader::load()
{
    ...
    if (content.empty()) {
        std::cerr << "ConfigurationReader: Failed to read file: " << config_filepath_ << std::endl;
        return false;
    }
    ...
    std::cout << "ConfigurationReader: Successfully loaded config from " << config_filepath_ << std::endl;
    return true;
}
...
std::string ConfigurationReader::get_executable_directory() const
{
    ...
    if (count == -1) {
        std::cerr << "ConfigurationReader: Failed to get executable path" << std::endl;
